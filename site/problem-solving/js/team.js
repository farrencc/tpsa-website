...
/* Brand marks for the profile links, on the same 16- and 24-unit grids the
   glyphs are drawn on. Static strings, so they are safe to insert as markup. */
const GLYPHS = {
...
/* The modal's link row, read off the card's data-* attributes.
   ...
   - data-linkedin / data-github   profile URLs; the button appears only when
     the card carries one, with data-linkedin-label / data-github-label
     overriding its text.
   Every label is overridable so that no button's wording has to be stitched
   together from a name that does not split into parts cleanly. */
